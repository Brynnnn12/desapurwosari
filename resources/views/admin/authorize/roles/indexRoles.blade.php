...
@section('content')
        ...
        <div x-show="open" @click.away="open = false" class="absolute z-10 bg-white p-4 rounded shadow-lg">
            <form action="{{ route('roles.store') }}" method="POST">
                ...
                <div class="mb-2">
                    ...
                    <input type="text" name="name" id="roles" required
                        class="border border-slate-200 rounded px-2 py-1 w-full" placeholder="Masukkan nama role">
                </div>
                <button type="submit"
                    class="inline-block px-3 py-1 text-sm font-bold bg-green-500 text-white rounded hover:bg-green-600">
                    Simpan
                </button>
    ...
    @click="
        edit = true;
        roles = {  name: '{{ $role->name }}', permission: @json($role->permissions->pluck('id')->toArray()) };
    "
